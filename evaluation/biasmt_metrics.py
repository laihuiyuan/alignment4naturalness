...
def textToLFP(unseen_text, sentences, step=1000, last=2000):
    ...
    unseen_freqs = FreqDist(" ".join(unseen_text).split())
    to_eval_freqs = FreqDist(" ".join(sentences).split())
    ...
    freqs = [unseen_freqs.most_common(size+step)[size:size+step] for size in sizes[:-1]]
    freqs.append(unseen_freqs.most_common()[last:])

    # Define words for top two bands
    band_1_words = set([word for word, _ in freqs[0]])
    band_2_words = set([word for word, _ in freqs[1]])

    # Total tokens
    totalCount=to_eval_freqs.N()

    # Counts frequency bands
    b1, b2, b3 = 0, 0, 0
    for (word, count) in to_eval_freqs.items():
        if word in band_1_words:
            b1 += count
        elif word in band_2_words:
            b2 += count
        else:
            b3 += count

    percs = [b1/totalCount, b2/totalCount, b3/totalCount]
